...
final RegExp signalTagPattern = RegExp(r'<npc-signal>\s*\{\s*"signal"\s*:\s*"([^"]+)"[\s\S]*?\}\s*</npc-signal>');
...
void main() {
  group('Prompt signal consistency test', () {
    for (final filePath in promptFiles) {
      test(
          'Signals used in text match signals listed in ## Signale section', () async {
      ...
      test('Full signal tags match documented signal tags in $filePath', () async {
        ...
        expect(
          ...
          reason: 'Full signal tags must match exactly in $filePath (including formatting and attributes).',
        );
      });
    }
  });
...
Set<String> extractFullSignalTags(String input, {bool onlyInSignaleSection = false}) {
  final signalTagPattern = RegExp(
    r'<npc-signal>\s*\{[\s\S]*?\}\s*</npc-signal>',
    caseSensitive: true,
  );

  final relevantText = onlyInSignaleSection
      ? input.split(RegExp(r'##\s*Signale', caseSensitive: false)).elementAtOrNull(1) ?? ''
      : input.split(RegExp(r'##\s*Signale', caseSensitive: false)).first;

  return signalTagPattern.allMatches(relevantText).map((m) => m.group(0)!.trim()).toSet();
}
...
/*
Set<String> extractSignalsFromSignaleSection(String prompt) {
  final signaleSectionRegex = RegExp(r'## Signale\s*\[.*?\]\s*((?:.|\n)*?)$', multiLine: true);
  final match = signaleSectionRegex.firstMatch(prompt);
  if (match == null) return {};

  final sectionText = match.group(1)!;
  final signalRegex = RegExp(r'<npc-signal>\{\s*"signal"\s*:\s*"([^"]+)"\s*\}</npc-signal>');
  return signalRegex.allMatches(sectionText).map((m) => m.group(1)!).toSet();
}*/
